...
export default function AddressesPage() {
  ...
  const filteredAddresses = useMemo(() => {
    ...
    return addresses.filter((address) => [address.city, address.district, address.street, address.building].some((value) => value.toLocaleLowerCase().includes(normalizedQuery)));
  }, [deferredQuery, addresses]);
    ...
    <main className="min-h-full bg-gray-50 p-6">
      <div className="space-y-5">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Manzillar</h1>
        ...
        {isError && <PageState className="text-red-500">Manzillarni yuklashda xatolik yuz berdi</PageState>}
        {!isPending && !isError && filteredAddresses.length === 0 && <PageState>Manzillar topilmadi</PageState>}
        {!isPending && !isError && filteredAddresses.length > 0 && (
          <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-label="Manzillar ro'yxati">
            {filteredAddresses.map((address) => <AddressCard key={address.id} address={address} />)}
          </section>
        )}
      </div>

      {isCreateOpen && (
        <div role="dialog" aria-modal="true" aria-labelledby="create-address-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 id="create-address-title" className="text-xl font-bold text-gray-900">Yangi manzil</h2>
                <p className="mt-1 text-sm text-gray-500">Manzil ma'lumotlarini kiriting.</p>
              </div>
            ...
            {createAddress.isError && <p className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-600">Manzilni yaratib bo'lmadi. Ma'lumotlarni tekshirib, qayta urinib ko'ring.</p>}
            <AddressForm
              isSubmitting={createAddress.isPending}
              onCancel={() => setIsCreateOpen(false)}
              onSubmit={(payload) => createAddress.mutate(payload, { onSuccess: () => setIsCreateOpen(false) })}
            />
          </div>
        </div>
      )}
