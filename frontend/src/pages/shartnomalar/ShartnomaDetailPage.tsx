...
const ShartnomaDetailPage = () => {
  ...
  if (loading) return <div className="flex min-h-full items-center justify-center bg-gray-50 p-6 text-sm text-gray-400">Yuklanmoqda...</div>;
  if (error || !contract) return <div className="flex min-h-full items-center justify-center bg-gray-50 p-6 text-sm text-red-500">Shartnomani yuklashda xatolik yuz berdi.</div>;
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {isEditing && contract ? (
          <div className="bg-white rounded-2xl p-6" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
            <h2 className="mb-4 text-lg font-bold text-gray-900">Shartnomani tahrirlash</h2>
            {updateContract.isError && <p className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-600">O'zgarishlarni saqlab bo'lmadi.</p>}
            <ContractForm
              initialContract={contract}
              submitLabel="Saqlash"
              isSubmitting={updateContract.isPending}
              onCancel={() => setIsEditing(false)}
              onSubmit={(payload: ContractPayload) => updateContract.mutate({ contractId: contract.id, payload }, { onSuccess: () => setIsEditing(false) })}
            />
          </div>
        ) : (
          <>
