...
export default function InvoicesPage() {
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return invoices.filter((invoice) => {
      ...
      const matchesQuery = !normalizedQuery || [invoice.invoice_number, invoice.document_doc_number, invoice.contract_number].some((value) => value.toLocaleLowerCase().includes(normalizedQuery));
      return matchesStatus && matchesQuery;
    });
  }, [deferredQuery, statusFilter, invoices]);
  ...
  return (
    <main className="min-h-full bg-gray-50 p-6">
      <div className="space-y-5">
        ...
        {isError && <PageState className="text-red-500">Invoicelarni yuklashda xatolik yuz berdi</PageState>}
        {!isPending && !isError && filteredInvoices.length === 0 && <PageState>Invoicelar topilmadi</PageState>}
        {!isPending && !isError && filteredInvoices.length > 0 && (
          <section className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" aria-label="Invoicelar ro'yxati">
      ...
      {isCreateOpen && (
        <div role="dialog" aria-modal="true" aria-labelledby="create-invoice-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <h2 id="create-invoice-title" className="text-xl font-bold text-gray-900">Yangi invoice</h2>
                <p className="mt-1 text-sm text-gray-500">Hisob-faktura ma'lumotlarini kiriting.</p>
              </div>
            ...
            {createInvoice.isError && <p className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-600">Invoice yaratib bo'lmadi. Ma'lumotlarni tekshirib, qayta urinib ko'ring.</p>}
            <InvoiceForm
              isSubmitting={createInvoice.isPending}
              onCancel={() => setIsCreateOpen(false)}
              onSubmit={(payload) => createInvoice.mutate(payload as InvoiceCreatePayload, { onSuccess: () => setIsCreateOpen(false) })}
            />
          </div>
        </div>
      )}
